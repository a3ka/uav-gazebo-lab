...
```bash
F=docs/v10_access/BFT_UAV_Swarm_Paper_v10_access.tex  # run from repo root
...
Then recompile in Docker so the PDF picks up the resolved DOI:
...
2. Upload a tarball of the entire `uav-gazebo-lab` repository state at tag `v1.0.0`:
   ```bash
   cd /path/to/parent-of-uav-gazebo-lab
