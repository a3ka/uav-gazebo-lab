...
- [x] Phase 6 — Progressive attrition + load balancing — Tier B (Python MC) COMPLETE 2026-05-24; v3 with anchor admission control + ATTACH-timeout retry: **14/15 trials PASS with 0 over_cap interval** (vs 0/15 baseline, 6/15 with jitter only). Mission survival 14/15. Capacity invariant fully restored. Tier A 1-trial Gazebo PASS at 6.10s.
- [x] Phase 7 — Ablation 2026-05-24; 4 ablations × 3-5 trials. ABL1 spoof-detection NECESSARY (98.3%→0%). ABL2 failover-watchdog is timing knob (6s→31s, 5×). ABL3 α_cap NOT load-balancing lever (now superseded by admission-control v3). ABL4-v3 with fixed scaffold: TRN marginal (~10%) in dense topology — Pillar 3 reframed.
- [x] Phase 8 — SwarmRaft baseline 2026-05-25 production comparison DONE. Median: **Raft 5.087s vs Ours 6.080s** (10 trials × N=5). ~1s gap reflects our additional per-follower capacity-coordination payload (REASSIGN/OFFER/ATTACH/ACK) that Raft does not provide. 4 bugs fixed during bring-up (BEST_EFFORT QoS, UDP-only DDS, awk scientific notation, Python heartbeat watcher).
